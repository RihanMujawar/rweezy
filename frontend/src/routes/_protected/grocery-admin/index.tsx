...
function GroceryDashboard() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    const [{ store: s, stats, orders: o }, { partners: p }] = await Promise.all([
      api.groceryAdmin.getDashboard(),
      api.groceryAdmin.getDeliveryPartners(),
    ]);
  ...
  const assignDelivery = async (orderId: string, deliveryBoyId: string) => {
    try {
      await api.groceryAdmin.assignDelivery(orderId, deliveryBoyId);
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1 space-y-8">
              ...
              {(alerts.lowStock.length > 0 || alerts.expiringSoon.length > 0) && (
                <div className="rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm dark:border-amber-800 dark:bg-amber-950/40 shadow-sm">
                  <p className="font-semibold text-amber-900 dark:text-amber-100">
                    Inventory alerts
                  </p>
                  {alerts.lowStock.length > 0 && (
                    <p className="mt-1">{alerts.lowStock.length} items low on stock.</p>
                  )}
                </div>
              )}
              ...
              <div className="rounded-2xl border bg-card p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                    ...
                    <Switch
                      checked={form.is_raining ?? false}
                      onCheckedChange={(v) => setForm({ ...form, is_raining: v })}
                    />
                  </div>
                  <hr />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        type="number"
                        value={form.packaging_fee ?? 0}
                        onChange={(e) =>
                          setForm({ ...form, packaging_fee: Number(e.target.value) })
                        }
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        type="number"
                        value={form.rain_fee ?? 0}
                        onChange={(e) => setForm({ ...form, rain_fee: Number(e.target.value) })}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input
                        type="number"
                        value={form.discount_flat ?? 0}
                        onChange={(e) =>
                          setForm({ ...form, discount_flat: Number(e.target.value) })
                        }
                      />
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={form.is_bogo_active ?? false}
                      onCheckedChange={(v) => setForm({ ...form, is_bogo_active: v })}
                    />
                  </div>
                      ...
                      <div key={order.id} className="p-6 hover:bg-muted/10 transition-colors">
                        ...
                        {!order.delivery_boy_id && order.status === "pending" && (
                          <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-950/20 rounded-2xl border border-orange-100 dark:border-orange-900/30">
                            <div className="text-sm font-semibold text-orange-800 dark:text-orange-400 mb-3">
                                ...
                                >
                                  <div className="text-left">
                                    ...
                                    <div className="text-[10px] opacity-70">
                                      {partner.status} • {partner.activeCount} active
                                    </div>
                                  </div>
                        ...
                        {order.delivery_boy_id && (
                          <div className="mt-4 flex items-center gap-3 p-3 bg-muted rounded-xl">
                            <div className="h-8 w-8 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-xs">
                              {partners.find((p) => p.id === order.delivery_boy_id)?.name?.[0] ||
                                "D"}
                            </div>
                            <div className="flex-1">
                              <div className="text-xs font-semibold">Assigned Partner</div>
                              <div className="text-sm">
                                {partners.find((p) => p.id === order.delivery_boy_id)?.name ||
                                  "Unknown Partner"}
                              </div>
                            </div>
