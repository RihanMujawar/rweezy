...
function StorePage() {
  ...
  const addItem = (item: Item) => {
    if (cart.storeId && cart.storeId !== storeId) {
      toast.message("Cart cleared", { description: "You can only order from one store at a time." });
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <Link to="/app/grocery" className="text-sm text-muted-foreground hover:underline">← All stores</Link>
      <div className="mt-2 flex items-start justify-between gap-4">
        ...
        {cartCount > 0 && cart.storeId === storeId && (
          <Button onClick={() => navigate({ to: "/app/grocery/checkout" })}>
