...
const NEXT: Record<string, string | null> = {
  ...
  accepted: "preparing",
  preparing: "ready",
  ready: null, // delivery boy takes over
};
...
function HotelOrders() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const { orders } = await api.hotel.getHistory();
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const { restaurantId: rid, orders } = await api.hotel.getOrders();
      if (rid) {
        setRestaurantId(rid);
        setOrders((orders as Order[]) ?? []);
      }
  ...
  const advance = async (order: Order) => {
    ...
    if (!next) return;
    try {
      await api.hotel.advanceOrder(order.id, next);
  ...
  const reject = async (order: Order) => {
    const reason = window.prompt("Reason for rejecting this order?", "Out of stock / too busy");
    if (reason === null) return;
    try {
      await api.hotel.rejectOrder(order.id, reason);
  ...
  const bulkAdvance = async (next: string) => {
    ...
    await Promise.all(targets.map((order) => api.hotel.advanceOrder(order.id, next)));
  ...
  const avgPrep = preparingOrders.length * 15 + readyOrders.length * 20;
      ...
      {showActions && (
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {["pending", "accepted"].includes(o.status) && (
            <Button size="sm" variant="destructive" className="min-h-11" onClick={() => reject(o)}>
              Reject
            </Button>
          )}
          {o.profiles?.phone ? (
          ...
          <Button
            ...
            onClick={() =>
              navigator.clipboard?.writeText(
                "Hi, your order is being prepared. We will update you shortly.",
              )
            }
          >
            Copy quick reply
        ...
        {loading ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-6">
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <div className="rounded-lg border bg-card p-4">
                <p className="text-xs text-muted-foreground">Prep load</p>
                <p className="mt-1 flex items-center text-2xl font-bold">
