...
const ASSIGNABLE: AppRole[] = ["admin", "hotel_manager", "grocery_manager", "delivery_boy", "rider"];
...
function AdminUsers() {
  ...
  const load = useCallback(async () => {
    const { profiles, roles } = await api.admin.getUsers();
  ...
  return (
    <RoleGate allowed={["admin"]} hasAny={roles.includes("admin")}>
      <div className="container mx-auto px-4 py-8">
        ...
        <Input className="mt-4 max-w-sm" placeholder="Search by name or ID..." value={search} onChange={(e) => setSearch(e.target.value)} />
